    ...
    return authAsync.when(
      data: (auth) {
        ...
        Widget currentScreen() {
          ...
          switch (auth.userRole) {
            case 'teacher':
              return roleScreens['teacher']?[selectedItem] ?? const SizedBox();
            case 'student':
              return roleScreens['student']?[selectedItem] ?? const SizedBox();
            case 'parent':
              return roleScreens['parent']?[selectedItem] ?? const SizedBox();
            case 'admin':
              return roleScreens['admin']?[selectedItem] ?? const SizedBox();
            default:
              return const SizedBox();
          }
        }
